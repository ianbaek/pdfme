...
const organize = async (pdf, actions) => {
    ...
    for (const action of actions) {
        ...
        switch (action.type) {
            case 'remove':
                currentPdf = await pdf_lib_1.PDFDocument.load(await (0, exports.remove)(currentBuffer, [action.data.position]));
                break;
            case 'insert':
                currentPdf = await pdf_lib_1.PDFDocument.load(await (0, exports.insert)(currentBuffer, [action.data]));
                break;
            case 'replace': {
                const withoutTarget = await (0, exports.remove)(currentBuffer, [action.data.position]);
                currentPdf = await pdf_lib_1.PDFDocument.load(await (0, exports.insert)(withoutTarget, [action.data]));
                break;
            }
            case 'rotate':
                currentPdf = await pdf_lib_1.PDFDocument.load(await (0, exports.rotate)(currentBuffer, action.data.degrees, [action.data.position]));
                break;
            case 'move':
                currentPdf = await pdf_lib_1.PDFDocument.load(await (0, exports.move)(currentBuffer, action.data));
                break;
            default:
