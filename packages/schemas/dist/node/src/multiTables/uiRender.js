...
const uiRender = async (arg) => {
    ...
    for (let rowIndex = 0; rowIndex < bodyWidthRange.length; rowIndex++) {
        ...
        for (let tableIndex = 0; tableIndex < tables.length; tableIndex++) {
            ...
            const table = await (0, tableHelper_js_1.createSingleTable)([tableRowData], { ...arg, schema: tableSchema }, tables.indexOf(singleTable));
            // Set table position
            ...
            if (mode === 'designer' && onChange) {
                ...
                singleTable.head.forEach((head, colIndex) => {
                    ...
                    const removeColumnButton = createButton({
                        ...
                        top: `${currentY}mm`,
                        left: `${offsetX - (0, common_1.px2mm)(columnButtonSize)}mm`,
                        text: '-',
                        onClick: () => {
