...
const embedAndGetFontObj = async (arg) => {
    ...
    const fontValues = await Promise.all(Object.entries(font).map(async ([key, v]) => {
        let fontData = v.data;
        if (typeof fontData === 'string' && fontData.startsWith('http')) {
            fontData = await fetch(fontData).then((res) => res.arrayBuffer());
        }
        return pdfDoc.embedFont(fontData, {
            subset: hasFillable && key === 'Roboto'
                ? false
...
const pdfRender = async (arg) => {
    ...
    if (schema.fillable) {
        ...
        if (schema.backgroundColor) {
            bg = (0, utils_js_1.hex2PrintingColor)(schema.backgroundColor, colorType);
        }
        ...
        textField.setAlignment(alignMap[alignment]);
        textField.defaultUpdateAppearances(pdfFontValue);
        form.acroForm.dict.set(pdf_lib_1.PDFName.of('NeedAppearances'), pdf_lib_1.PDFBool.True);
        return;
    }
    if (schema.backgroundColor) {
...
const alignMap = {
    left: pdf_lib_1.TextAlignment.Left,
    center: pdf_lib_1.TextAlignment.Center,
    right: pdf_lib_1.TextAlignment.Right,
};
