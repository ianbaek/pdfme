...
const pdfRender_js_1 = require("../barcodes/pdfRender.js");
const image_js_1 = __importDefault(require("../graphics/image.js"));
const constants_js_1 = require("../barcodes/constants.js");
const linePdfRender = line_js_1.default.pdf;
const cellBasePdfRender = cell_js_1.default.pdf;
const imagePdfRender = image_js_1.default.pdf;
async function drawCell(arg, cellInfo) {
    ...
    if (typeof cellContent === 'string') {
        ...
        else if (constants_js_1.BARCODE_TYPES.includes(cellContent.type) && pdfRender_js_1.pdfRender) {
...
async function drawTableBorder(arg, table, startPos, cursor, isLastTable = true) {
...
async function drawTable(arg, table, isLastTable = true) {
...
function convertSingleTableToTableSchema(singleTable, multiTableSchema) {
    return {
        ...multiTableSchema,
...
const pdfRender = async (arg) => {
    ...
    // Get content data
    const content = (0, helper_js_1.getBodyWithRange)(typeof value !== 'string' ? (value ? JSON.stringify(value) : '[]') : value || '[]', schema.__bodyRange);
    ...
    for (let rowIndex = 0; rowIndex < content.length; rowIndex++) {
        ...
        for (const singleTable of tables) {
            ...
            const table = await (0, tableHelper_js_1.createSingleTable)([tableRowData], createTableArgs, tables.indexOf(singleTable));
            // Set table position
