...
import type { MultiTableSchema, SingleTableSchema, CellStyle, Styles } from './types.js';
    ...
    for (let tableIndex = 0; tableIndex < tables.length; tableIndex++) {
      ...
      if (mode === 'designer' && onChange) {
        ...
        singleTable.head.forEach((head, colIndex) => {
          if (singleTable.head.length === 1) return;
          ...
          const removeColumnButton = createButton({
            ...
            onClick: () => {
              const totalWidthMinusRemoved = singleTable.headWidthPercentages.reduce(
                (sum, width, j) => (j !== colIndex ? sum + width : sum),
                0,
              );
              ...
              const currentContent = JSON.parse(value || '[]') as string[][];
      ...
      if (mode === 'designer' && onChange && rowIndex === 0 && tables.length > 1) {
        ...
        const addColumnButton = createButton({
          ...
          onClick: (e) => {
            ...
            const totalCurrentWidth = singleTable.headWidthPercentages.reduce(
              (acc, width) => acc + width,
              0,
            );
