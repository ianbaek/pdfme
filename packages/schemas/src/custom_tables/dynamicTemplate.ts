...
import { CustomTableSchema } from './types.js';

export const getDynamicHeightsForCustomTable = async (
  value: string,
  args: {
    schema: Schema;
    basePdf: BasePdf;
  ...
  if (args.schema.type !== 'customTable') return Promise.resolve([args.schema.height]);
  const schema = args.schema as CustomTableSchema;
  const body =
